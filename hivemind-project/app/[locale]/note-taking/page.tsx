...
const formatRelativeTime = (date: Date) => {
    const now = new Date();
    const diff = Math.floor((now.getTime() - date.getTime()) / 1000);  // คำนวณเวลาต่างกันเป็นวินาที
    if (diff < 60) return "just now"; // น้อยกว่า 1 นาที
    if (diff < 3600) return `${Math.floor(diff / 60)} minutes ago`; // ภายในชั่วโมง
    if (diff < 86400) return `${Math.floor(diff / 3600)} hours ago`; // ภายในวัน
    if (diff < 604800) return `${Math.floor(diff / 86400)} days ago`;  // ภายในสัปดาห์
