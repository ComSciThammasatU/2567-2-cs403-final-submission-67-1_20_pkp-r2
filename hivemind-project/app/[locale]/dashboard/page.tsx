...
export default function Dashboard() {
    ...
    const unsubscribeProjects = onSnapshot(projectsQuery, (projectsSnap) => {
      ...
      getDocs(tasksQuery).then((tasksSnap) => {
        ...
        tasksSnap.forEach(doc => {
          const task = doc.data(); // ดึงข้อมูลของแต่ละ task จาก snapshot
  ...
  // สร้าง array วันที่ย้อนหลัง 7 วันสำหรับ daily task
  const getLast7Days = () => {
    const dates = [];
    for (let i = 0; i < 7; i++) {
      const date = new Date();
  ...
  // เตรียมวันที่ย้อนหลัง 7 วัน และ state วันที่เลือก
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow w-full">
              <h3 className="text-xl font-semibold mb-4">{t("task_today")}</h3>
